...
В качестве примера, предположим, что у нас **есть парсер `int`**, который распознает целочисленные __литералы__ 
и **парсер `плюс`**, который распознает символ _`«+»`_. Следовательно, мы можем **создать парсер**, 
